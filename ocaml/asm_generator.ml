...
let rec exp_to_asm (exp : exp) (regf : Register_alloc.registers_function)  (resr : string) : string  = (*expression, variable->registers, register to put the result in*)
 match exp with 
	...
	| Ident i ->  sprintf "MOV R0, %s" (regf#look_for i)
	| Label s -> sprintf "%s" (s)
	| Neg i -> sprintf "MOV R1, #%s \n  NEG R1, R1 " i 
	| FNeg i ->  sprintf " "
	| Add (i,id) -> sprintf "ADD %s , %s , %s" (resr) (ident_or_imm_to_asm id regf) (regf#look_for i)
	| Sub (i,id) -> sprintf "SUB %s , %s, %s " (resr) (regf#look_for i) (ident_or_imm_to_asm id regf ) 
	...
	| FMul (i,id) -> sprintf " "
	| FDiv (i,id) -> sprintf " "
	| New i -> sprintf " MOV %s, new %s " (resr ) (ident_or_imm_to_asm i regf)
